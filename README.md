...
- **It judges sustained load, not the latest reading.** The level comes from the
  median of the last ~16 seconds. Single 100% samples happen constantly, on every
  app launch and every page load, and a menu bar that flashes red at each one is
...
3. `SIGTERM`/`SIGINT`/`SIGHUP`/`SIGQUIT`, meaning logout, shutdown and `killall`, are
...
- **Processes macOS needs are protected outright**, including the Dock, `loginwindow`,
